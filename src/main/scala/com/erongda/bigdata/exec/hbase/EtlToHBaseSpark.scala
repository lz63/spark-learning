...
object EtlToHBaseSpark {
  ...
  def createRowKey(time: Long, uUID: String, uMD: String, eventAlias: String): String = {
    ...
    sBuffer.append(crc32.getValue % 10000000L)
    sBuffer.toString
  }
  ...
  def createHBaseTable(processDate: String, conf: Configuration): String = {
    ...
    try{
      ...
      // 创建表的列簇描述符
      val familyDesc = new HColumnDescriptor(EventLogConstants.EVENT_LOGS_FAMILY_NAME)
  ...
  def main(args: Array[String]): Unit = {
   ...
   val sparkConf = new SparkConf()
    ...
    /**
      ...
      */
    val eventLogsRDD = sc.textFile(s"/datas/${args(0)}/20151220.log",minPartitions = 3)
    ...
    /**
      * TODO: b. 解析每条日志数据
      */
    val eventParserRDD: RDD[(String, util.Map[String, String])] = eventLogsRDD
      // 通过解析工具类解析每条数据
      .map(
      line => {
        // 调用工具类进行解析得到Map集合
    ...
    // 存储事件类型EventType
    val eventTyteList = List(
      EventEnum.CHARGEREFUND, EventEnum.CHARGEREQUEST, EventEnum.CHARGESUCCESS,
      EventEnum.EVENT, EventEnum.LAUNCH, EventEnum.PAGEVIEW
    )
    // TODO: 通过广播变量将事件类型的列表广播给各个Executor
    val eventTypeListBroadcast: Broadcast[List[EventEnum]] = sc.broadcast(eventTyteList)

    /**
      * TODO：c. 过滤数据
      */
    val eventPutsRDD: RDD[(ImmutableBytesWritable, Put)] = eventParserRDD
      ...
      val rowKey: String = createRowKey(
        TimeUtil.parseNginxServerTime2Long(EventLogConstants.LOG_COLUMN_NAME_SERVER_TIME), //
    ...
    println(s"eventCount = ${eventPutsRDD.count()}")
    println(s"first = ${eventPutsRDD.first()}")
    /**
