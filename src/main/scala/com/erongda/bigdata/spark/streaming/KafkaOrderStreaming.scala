      ...
      if(!rdd.isEmpty()){
        ...
        rdd.coalesce(3).foreachPartition(iter => {
          ...
          try{
            ...
            props.put(ConstantUtils.METADATA_BROKER_LIST_NAME, ConstantUtils.METADATA_BROKER_LIST)
            props.put(ConstantUtils.SERIALIZER_CLASS_NAME, ConstantUtils.NEW_SERIALIZER_CLASS)
            props.put(ConstantUtils.KEY_SERIALIZER_CLASS_NAME, ConstantUtils.NEW_SERIALIZER_CLASS)
            // 生产者Producer实例对象
            ...
            iter.foreach{
              case (provinceId, order) =>
                val record = new ProducerRecord[String, String](
                  ConstantUtils.ORDER_TOPIC, order.orderId.toString, order.toString
                )
                // 发送数据
