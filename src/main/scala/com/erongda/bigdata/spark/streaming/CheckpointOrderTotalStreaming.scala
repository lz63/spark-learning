...
/**
  ...
  *     实时累加统计 各省份销售订单的总销售额
  */
object CheckpointOrderTotalStreaming {

  // 检查点目录
  val CHECKPOINT_PATH = "/datas/sparkstreaming/ckpt-111111111111111"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val context = StreamingContext.getActiveOrCreate(
      CHECKPOINT_PATH, //
      // 实时流式应用第一次运行的时候，使用下面的函数创建StreamingContext实例对象，接收数据源的数据，处理数据并输出
      () => {
        // 创建SparkConf实例对象，设置应用相关信息
        val sparkConf = new SparkConf()
          .setMaster("local[3]")
          .setAppName("CheckpointOrderTotalStreaming")
          // 从每个Kafka分区读取数据的最大速率(每秒记录数)
        ...
        /**
          * 设置检查点目录，功能；
          *   -a. 保存状态信息（updateStageByKey)
          *   -b. 保存每次 从kafka topic 中读取数据的OFFSET
          *   -c. 保存DStream来源、DStream数据处理函数与输出函数
          */
    ...
    context.start() // 将会启动Receiver接收器，用于接收数据源端的数据
    // 实时应用一旦运行起来，正常情况下不会自动的停止，除非遇到特殊情况，如认为终止程序，或者程序出现异常，需要等待
    context.awaitTermination()
    // 停止StreamingContext
    // 第一个参数：停止关联的SparkContext。底层SparkContext将被停止，不管这个StreamingContext是否已经被停止
    // 第二个参数：收到待完成数据，优雅地停下来等待所有的进程
    context.stop(stopSparkContext = true, stopGracefully = true)
  }

  /**
    * 采用Direct方式从Kafka Topic拉取数据，实时累加统计各省份销售订单额
    * @param ssc
    */
  def processData(ssc: StreamingContext): Unit = {
    ...
    /**
      ...
      */
    val orderDStream: DStream[(String, Double)] = kafkaDStream
      .map(_._2)
      .transform(rdd => {
        rdd
          // 表示过滤不合格的数据
    ...
    val priceDStream: DStream[(String, Double)] = orderDStream.updateStateByKey(
      (values: Seq[Double], state: Option[Double]) => {
        ...
        // 3. 合并状态并返回
        // Some将返回原来的类型，此处返回orderDStream的类型
