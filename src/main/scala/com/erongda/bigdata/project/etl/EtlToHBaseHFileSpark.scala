...
object EtlToHBaseHFileSpark {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      * TODO：c. 过滤数据
      */
    val eventPutsRDD: RDD[(ImmutableBytesWritable, KeyValue)] = parseEventLogsRDD
      ...
      .flatMap{ case (eventAlias, logInfo) =>
        // -i. RowKey表的主键
        ...
        val rkimw = new ImmutableBytesWritable(rk)
        // -ii. 创建Put对象，添加相应列
        val cellList = new ListBuffer[(ImmutableBytesWritable, KeyValue)]()
    ...
    // 设置输出的HFile文件的目录
    val outputDir: Path = new Path( "/datas/spark/hbase/etlhfile" + System.currentTimeMillis)
    ...
    if(FileSystem.get(conf).exists(outputDir)){
      // 加载生成的HFile文件
      val load: LoadIncrementalHFiles = new LoadIncrementalHFiles(conf)
      load.doBulkLoad(outputDir, table)
    }
