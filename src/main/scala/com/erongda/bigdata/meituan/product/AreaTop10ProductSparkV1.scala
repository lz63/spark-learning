...
import com.erongda.bigdata.meituan.util.ParamUtils
...
/**
  ...
  */
object AreaTop10ProductSparkV1 {
...
*/
  ...
  /**
    ...
    *              SparkSession会话实例对象
    */
  def persistAreaTop10CountFullProductData(spark: SparkSession): Unit ={
    /**
    ...
    df.foreachPartition(iter => {
      Try{
        ...
        /**
          ...
          */
        val sqlStr = "INSERT INTO tb_area_top10_product(`area`, `product_id`, `area_level`, `count`, `city_infos`, `product_name`, `product_type`) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE `area_level` = VALUES(`area_level`), `count` = VALUES(`count`),`city_infos` = VALUES(`city_infos`),`product_name` = VALUES(`product_name`),`product_type` = VALUES(`product_type`)"
        ...
        iter.foreach(row => {
          ...
          pstmt.setString(3, areaLevel)
          pstmt.setLong(4, clickCount)
          pstmt.setString(5, cityInfos)
  ...
  def main(args: Array[String]): Unit = {
    ...
    persistAreaTop10CountFullProductData(spark)
