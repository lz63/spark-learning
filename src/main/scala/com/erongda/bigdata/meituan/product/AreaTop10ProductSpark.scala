package com.erongda.bigdata.meituan.product

import java.sql.{Connection, DriverManager, PreparedStatement, ResultSet}
import java.util.Properties

import com.alibaba.fastjson.{JSON, JSONObject}
import com.erongda.bigdata.meituan.dao.JDBCHelper
import com.erongda.bigdata.meituan.mock.MockDataUtils
import com.erongda.bigdata.meituan.util._
...
object AreaTop10ProductSpark {
  ...
  /**
    ...
    *         DataFrame数据集
    */
  def getActionDataFrameByFilter(spark: SparkSession,
                                 jsonObject: JSONObject): DataFrame = {
    ...
    /**
      ...
      *
      *     {"sex":"male","startDate":"2017-03-01","endDate":"2017-03-20","professionals":"程序员"}
      */
    // 获取参数数据
    val startDateOption: Option[String] = ParamUtils.getParam(jsonObject, TrackConstants.PARAM_START_DATE)
    val endDateOption: Option[String] = ParamUtils.getParam(jsonObject, TrackConstants.PARAM_END_DATE)
    // a. 构建SQL语句
    val sql =
      s"""
        |SELECT
  ...
  def getCityInfoDataFrame(spark: SparkSession): DataFrame = {
    ...
    val tableName = TrackConstants.MYSQL_CITY_INFO_TABLE_NAME
    val props = new Properties()
    props.put("user", ConfigurationManager.getProperty(TrackConstants.JDBC_USER))
    props.put("password", ConfigurationManager.getProperty(TrackConstants.JDBC_PASSWORD))
...
*/
  ...
  /**
    ...
    *              SparkSession会话实例对象
    */
  def persistAreaTop10CountFullProductData(spark: SparkSession,
                                           taskId: Long): Unit ={
    /**
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (taskId, taskParam) = {
      ...
      //
      // val taskParamStr: String = "{\"sex\":\"male\",\"startDate\":\"2017-03-01\",\"endDate\":\"2017-03-20\",\"professionals\":\"程序员\"}"
      val taskParamStr: String = {
        var taskParamValue = ""
        // 定义变量
        ...
        try{
          // 获取连接
          conn = JDBCHelper.getMySQLConnection
          // 查询SQL
          val sqlStr = "SELECT task_param FROM tb_task WHERE task_id = ?"
          ptsmt = conn.prepareStatement(sqlStr)
          ptsmt.setInt(1, taskID.toInt)
          // 执行查询
          result = ptsmt.executeQuery()
          // 获取taskparam
          if(result.next()) taskParamValue = result.getString(1)
        }catch {
          case e: Exception => e.printStackTrace()
        }finally {
          if(null != result) result.close()
          if(null != ptsmt) ptsmt.close()
          if(null != conn) conn.close()
        }
        taskParamValue
      }
      // 解析JSON格式数据
      val taskParamJson: JSONObject = ParamUtils.getTaskParam(taskParamStr)
      if(null == taskParamJson){
        throw new IllegalArgumentException("无法获取数据过滤参数")
      }
      // 返回值
      (taskID, taskParamJson)
    }
    ...
    val actionDataFrame = getActionDataFrameByFilter(spark, taskParam)
