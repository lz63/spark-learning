package com.erongda.bigdata.meituan.ad
...
/**
  * 广告点击流量实时统计分析: 对收集得到的用户点击广告数据进行数据分析，并将结果保存到Redis/MySQL中
    ...
    *- 结果保存: Redis/MySQL
  */
object AdClickRealTimeStateSparkV5 {
  ...
  def calculateProvinceTop5Ad(aggDStream: DStream[(StateDimension, Int)]): Unit ={
    ...
    /**
      ...
      *   名称：tb_top5_province_ad_click_count
      *   字段：
      *     date、province、ad_id、clickCount
      *   插入方式：
      *     插入 Or 更新
      */
      ...
      if(!rdd.isEmpty()){
        ...
        rdd.coalesce(2).foreachPartition(iter => {
          Try{
            ...
            // 2. 创建Statement对象
            val sqlStr = "INSERT INTO tb_top5_province_ad_click_cnt(`rank_id`, `date`, `province`, `ad_id`, `click_count`) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE `ad_id`=VALUES(`ad_id`), `click_count`=VALUES(`click_count`)"
            val pstmt: PreparedStatement = conn.prepareStatement(sqlStr)

            // 3. 对数据进行迭代输出操作
            var recordCounter = 0
            iter.foreach{ case ((date, province), list) =>
              for(((adId, clickCount), index) <- list.zipWithIndex){
                // 设置参数
      ...
      if(!rdd.isEmpty()){
        ...
        rdd.coalesce(2).foreachPartition(iter => {
          Try{
            ...
            // TODO: 清空表中的前一个窗口统计的数据
            conn
              .prepareStatement("truncate table tb_ad_click_count_window")
              .execute()
            conn.commit()

            // 2. 创建Statement对象
            val sqlStr = "INSERT INTO tb_ad_click_count_window(`start_window_time`, `end_window_time`, `ad_id`, `click_count`) VALUES (?, ?, ?, ?)"
            val pstmt: PreparedStatement = conn.prepareStatement(sqlStr)
            ...
            iter.foreach{ case (adId, clickCount) =>
                // 设置参数
                pstmt.setString(1, startWindowTime)
                pstmt.setString(2, endWindowTime)
                pstmt.setInt(3, adId)
                pstmt.setInt(4, clickCount)
