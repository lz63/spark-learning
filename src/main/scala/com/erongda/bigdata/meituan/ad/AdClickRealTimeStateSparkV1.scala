...
/**
  * 广告点击流量实时统计分析: 对收集得到的用户点击广告数据进行数据分析，并将结果保存到Redis/MySQL中
    ...
    *- 结果保存: Redis/MySQL
  */
object AdClickRealTimeStateSparkV1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // TODO：六、实时累加广告点击量
    val aggregateDStream: DStream[((String, String, String, Int), Int)] = calculateRealTimeState(filterAdClickDStream)
  ...
  /**
    ...
    *                  过滤后的广告点击流量数据
    */
  def calculateRealTimeState(adDStream: DStream[AdClickRecord]): DStream[((String, String, String, Int), Int)] = {
    ...
    val aggregateDStream: DStream[((String, String, String, Int), Int)] = mappedDStream
      ...
      // 提取值
      .map{ case ((date, province, city, adId), (_, clickCount)) =>
    ...
    aggregateDStream.foreachRDD(rdd => {
      ...
      rdd.coalesce(2).foreachPartition(iter => {
        Try{
          ...
          // 3. 对数据进行迭代输出操作
          var recordCounter = 0
          iter.foreach{ case  ((date, province, city, adId), clickCount) =>
            // 设置参数
