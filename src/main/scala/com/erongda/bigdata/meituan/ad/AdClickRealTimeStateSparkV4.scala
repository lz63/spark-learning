...
/**
  * 广告点击流量实时统计分析: 对收集得到的用户点击广告数据进行数据分析，并将结果保存到Redis/MySQL中
    ...
    *- 结果保存: Redis/MySQL
  */
object AdClickRealTimeStateSparkV4 {
      ...
      if(!rdd.isEmpty()){
        ...
        rdd.coalesce(2).foreachPartition(iter => {
          Try{
            ...
            // 2. 创建Statement对象
            val sqlStr = "INSERT INTO tb_top5_province_ad_click_count2(`rank_id`, `date`, `province`, `ad_id`, `click_count`) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE `ad_id`=VALUES(`ad_id`), `click_count`=VALUES(`click_count`)"
            val pstmt: PreparedStatement = conn.prepareStatement(sqlStr)
  ...
  def calculateAdClickCountByWindow(adDStream: DStream[AdClickRecord]): Unit ={
    ...
    // b. 窗口聚合统计
    val adWindowDStream = adTupleDStream.reduceByKeyAndWindow(
    ...
    adWindowDStream.foreachRDD((rdd, time) => {
      if(!rdd.isEmpty()){
        // 获取批次时间，格式转换
        val batchTime = DateUtils.parseLong2String(time.milliseconds, "yyyy/MM/dd HH:mm:ss ")
