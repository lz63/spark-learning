...
/**
  * 广告点击流量实时统计分析: 对收集得到的用户点击广告数据进行数据分析，并将结果保存到Redis/MySQL中
    ...
    *- 结果保存: Redis/MySQL
  */
object AdClickRealTimeStateSparkV3 {
  ...
  def calculateProvinceTop5Ad(aggDStream: DStream[(StateDimension, Int)]): Unit ={
    ...
    /**
      ...
      *     插入 Or 更新
      */
    top5ProvinceAdClickCountDStream.foreachRDD((rdd, time) => {
      if(!rdd.isEmpty()){
        // RDD 缓存
        rdd.persist(StorageLevel.MEMORY_AND_DISK)

        val batchDate: String = new SimpleDateFormat("yyyyMMdd")
          .format(new Date(time.milliseconds))

        // TODO: 针对 各省份Top5广告点击数据，入库之前先删除，再插入
        Try {
          // 1.获取链接
          val conn = JDBCHelper.getMySQLConnection
          // 2.经过分析得到，结果插入数据之前，先删除
          conn
            .prepareStatement(s"delete from tb_top5_province_ad_click_count where date = '$batchDate'")
            .execute()
          // 3. 返回连接，用于关闭
          conn
        }match {
          case Success(mySQLConn) => if(null != mySQLConn) mySQLConn.close()
          case Failure(exception) => throw exception
        }
        ...
        rdd.coalesce(2).foreachPartition(iter => {
          Try{
            ...
            // 2. 创建Statement对象
            val sqlStr = "INSERT INTO tb_top5_province_ad_click_count(`date`, `province`, `ad_id`, `click_count`) VALUES (?, ?, ?, ?)"
            val pstmt: PreparedStatement = conn.prepareStatement(sqlStr)

            // 3. 对数据进行迭代输出操作
            var recordCounter = 0
            iter.foreach{ case ((date, province), list) =>
              for((adId, clickCount) <- list){
                // 设置参数
                pstmt.setString(1, date)
                pstmt.setString(2, province)
                pstmt.setInt(3, adId)
